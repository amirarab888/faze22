...
void myLogin()
{
    ...
    //authToken[numOfOnlineUsers]+1
    if(strcmp((passwordJSONString -> valuestring), firstAuthToken) != 0)
    {
...
void sendMessage()
{
    ...
    fprintf(filePtr, "%s", savingStringFromJSON);
    fclose(filePtr);
...
void indexOfReqUserFinder()
{
    for(int i=0; i<numOfOnlineUsers; i++)
    {
        ...
        if(strncmp(firstAuthToken, authToken[i], 7) == 0)
        {
            indexOfReqUser = i;
            return;
        }
